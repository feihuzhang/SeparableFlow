...
class InputPadder:
    ...
    def pad(self, *inputs):
        if self.mode == 'sintel':
            ...
            return [F.interpolate(x, [self.ht_new, self.wd_new], mode='bilinear', align_corners=True) for x in inputs]
    ...
    def unpad(self,x):
        if self.mode == 'sintel':
            ...
            flow = F.interpolate(x.unsqueeze(0), [self.ht, self.wd], mode='bilinear', align_corners=True)
